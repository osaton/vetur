...
type SnippetSource = 'workspace' | 'user' | 'stage';
...
export class SnippetManager {
  ...
  constructor(workspacePath: string, globalSnippetDir?: string) {
    const workspaceSnippets = loadAllSnippets(path.resolve(workspacePath, '.vscode/stage/snippets'), 'workspace');
    const userSnippets = globalSnippetDir ? loadAllSnippets(globalSnippetDir, 'user') : [];
    const stageSnippets = loadAllSnippets(path.resolve(__dirname, './stageSnippets'), 'stage');

    this._snippets = [...workspaceSnippets, ...userSnippets, ...stageSnippets];
  }
...
function loadAllSnippets(rootDir: string, source: SnippetSource): Snippet[] {
  ...
  try {
    fs.readdirSync(rootDir).forEach(p => {
      ...
      if (!absPath.endsWith('.stage') && fs.existsSync(absPath) && fs.lstatSync(absPath).isDirectory()) {
